      ...
      <div className="grid md:grid-cols-2 gap-8">
        {roadmap.map((weekData, idx) => (
          ...
          >
              ...
              <ul className="space-y-4">
                {(weekData.tasks || []).map((task, taskIdx) => (
                  <li key={taskIdx} className="flex gap-4 items-start group/item">
                    <div className="w-5 h-5 rounded-full bg-stone-100 dark:bg-white/5 flex items-center justify-center mt-0.5 group-hover/item:bg-indigo-500 group-hover/item:text-white transition-all">
            ...
            {/* Decorative element */}
            <div className="absolute -bottom-10 -right-10 w-32 h-32 bg-indigo-500/5 blur-3xl rounded-full group-hover:bg-indigo-500/10 transition-colors"></div>
          </motion.div>
        ))}
      </div>
      
      <div className="bg-indigo-500 p-8 rounded-[32px] text-white flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden relative">
        <div className="relative z-10">
